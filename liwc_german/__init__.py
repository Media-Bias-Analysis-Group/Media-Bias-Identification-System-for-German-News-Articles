def read_dic(filepath):
    ...
    * `categories` is a list of category names (as strings)
    ...
    mode = 0
    import codecs
    for line in codecs.open(filepath, 'r', encoding='utf8'):
        tsv = line.strip()
        if tsv:
            parts = tsv.split('\t')
            if parts[0] in str('﻿%') or parts[0] in '﻿%':
                mode += 1
            elif mode == 1:
                # defining categories
...
def load_token_parser(filepath):
    ...
    * `category_names` is a list of strings representing all LIWC categories in
    ...
    def parse_words(token, dic):
        for category_name in _search_trie(trie, token):
                if category_name not in dic:
                    dic[category_name] = [token]
                else:
                    dic[category_name].append(token)
    return parse_token, parse_words
