...
if __name__ == '__main__':
    with open(args.raw, 'r',encoding="UTF-8",errors="replace") as infile:
    # start pre processing with multiple threads
        pool = mp.Pool(args.threads)
        values = pool.imap(process_line, infile, chunksize=args.batch_size)
        with open(args.target, 'w',encoding="UTF-8", errors="replace" ) as outfile:
...
class CorpusSentences:
    ...
    def __iter__(self):
        for line in open(self.filename, errors="replace"):
            yield line.split()
...
if __name__ == '__main__':
    ...
    with open('{}.bigram'.format(args.target), 'w', errors="replace" ) as outfile:
